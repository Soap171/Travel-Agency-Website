...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    function Pointer(this: PointerType) {
      this.id = -1;
      this.texcoordX = 0;
      this.texcoordY = 0;
    ...
    const pointers = [new (Pointer as any)()] as PointerType[];

    const { gl, ext: extNullable } = getWebGLContext(canvas);
    if (!gl || !extNullable) return;
    const ext = extNullable;
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): { gl: WebGL2RenderingContext | WebGLRenderingContext | null; ext: GLExtensions | null } {
      const params = {
        alpha: true,
        depth: false,
        stencil: false,
    ...
    function compileShader(type: number, source: string, keywords?: string[]): WebGLShader | null {
      const parsedSource = addKeywords(source, keywords);
      const shader = gl!.createShader(type);
      if (!shader) return null;
      gl!.shaderSource(shader, parsedSource);
  ...
  return (
    <div
      style={{
        ...
        top: 0,
        left: 0,
        zIndex: isAbsolute ? 15 : 9999, // Lower z-index for layered absolute hero overlay, high for global
        pointerEvents: "none",
        width: "100%",
        height: "100%",
        overflow: "hidden",
